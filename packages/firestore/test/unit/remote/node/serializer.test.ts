...
// TODO(mrschmidt): Passing in `null` only works because these teste don't
...
describe.only('Serializer', () => {
  ...
  describe('converts value', () => {
    ...
    function verifyFieldValueRoundTrip(opts: {
      ...
      const actualJsonProto = wrap(value);
      ...
      const actualReturnFieldValue = userDataWriter.convertValue(actualJsonProto);
  ...
  //
    ...
    it('converts TimestampValue from string', () => {
      expect(
        userDataWriter.convertValue({ timestampValue: '2017-03-07T07:42:58.916123456Z' })
      ).to.deep.equal(
       new Timestamp(1488872578, 916123456)
      );

      expect(
        userDataWriter.convertValue({ timestampValue: '2017-03-07T07:42:58.916123Z' })
      ).to.deep.equal(
        new Timestamp(1488872578, 916123000)
      );

      expect(
        userDataWriter.convertValue({ timestampValue: '2017-03-07T07:42:58.916Z' })
      ).to.deep.equal(
        new Timestamp(1488872578, 916000000)
      );

      expect(
        userDataWriter.convertValue({ timestampValue: '2017-03-07T07:42:58Z' })
      ).to.deep.equal(
